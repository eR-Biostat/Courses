...
the population mean. If a different random sample of 153 days were taken the new sample mean would likely be different as a result of sampling variation. Note that while estimates generally vary from one sample to another, the population mean ($\mu$) and variance $(\sigma^{2})$ are fixed values (the parameters in the population).
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Sample means of random samples from $N(0,1)$."}
plot(ni,m.x,type="l")
abline(0,0,col=2)
```
...
X is called the sampling distribution of the sample mean, and has its own mean and standard deviation. The concept of a sampling distribution
...
nearly normal. This result follows from the Central Limit Theorem.
...
deviation ($\sigma_{X}$) by the square root of the sample size $n$. Since the population standard deviation is typically unknown, the sample standard deviation $s$ is often used in the definition of a standard
...
around the population mean $\mu$, allowing for more accurate estimates of $\mu$ from a single sample, as
...
SD.wind
```
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Histogram of wind speed and confidence interval for the population mean."}
ggplot(airquality, aes(x = Wind)) +
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="The 0.005 abd 0.995 quantiles of $N(0,1)$ (the red lines)."}
qx1<-qnorm(0.005,0,1)
qx1
plot(x,dx,type="l",ylab="f(x)")
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Histogram with density plot for the BMI."}
ggplot(NHANES, aes(x = BMI)) +
  geom_histogram(aes(y = ..density..), fill = "skyblue", color = "black") +
  geom_density(alpha = 0.2, fill = "orange")+
  ylab("Density")
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Violin plot for the BMI."}
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Histogram and violin of a random sample for the BMI (n=50)."}
...
In this section we focus on the wind speed in the <tt>airquality</tt> data. The claim to be tested is that the population mean of the average wind speed is equal to 9 mph. 
$$ H_{0} : \mu = 9$$.
This implies that under the null hypothesis, the population mean $\mu=\mu_{0}=9$. In the absence of prior evidence that the wind speed is slower or faster, 
...
Recall that for the <tt>airquality</tt> dataset $n=153$ with sample mean and variance equal to $\bar{x}=9.957516$ and $s^{2}=3.523001$. Figure 9 shows the distribution of the wind speed.
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Illustraion of the p-value. Red line: the observed test statistic"}
...
determines the value of $z_{\alpha}$ that is used to calculate the margin of error $m = z_{\alpha} \times SE$.
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Boxplot and violin plot of the total cholestrol level."}
...
For a sample of 100 individuals, shown in Figure 26, $\bar{x}=5.0041$ and $s=1.20723$. This implies that the standard error of the sample means is equal to
...
```{r, echo=TRUE, message=FALSE, warning=FALSE,fig.cap="Histogram of the total cholesterol and a 95% confidence interval for the population mean."}
