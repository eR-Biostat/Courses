...
subtitle:  First steps of programing in R
...
This interactive book contains all R code that was used to produce the results and output presented in chapter 3 (programming) in the course's slides. We include YouTube tutorials as a part of the book and links to the relevant tutorials are provided in different chapters. Note that these tutorials were not developed especially for this book, they cover the same topics using different examples. 

## R ?
No previous knowledge about R is required. We start from the basic and follow a user approach and not a programmer approach. The datasets used for illustrations are available in R and one of them (the law school data) is part of the \texttt{ bootstrap} R package. To run the code smoothly, this package need to be installed.
...
```
 ...
 We can print <tt> x </tt>

```{r}
print(x)
```

The object <tt> x </tt> can be a vector 
...
The data structure that will be discussed in this chapter are
...
Another important concept in R is a function. For example, if we want to calculate the mean of a sample we can use the function <tt> mean() </tt> 

```{r}
mean(x)
```

This will be discuused from Chapter 3 onward.


## Scaler

The most simple object in Splus is a scaler. In order to define $X=5$ we use

```{r}
x <- 5  
```
 
 <tt> x </tt> is an object and the value of this object is 5. In order to see the object we type
<tt> x </tt>
...
A vector in R can be defined by using the function <tt> c() </tt>.For example the vector $x=(10,15,15,25,100)$ can be defined in the following way
...
Define two vectors: $x=(1,2,3,4,5)$ and $y=(10,15,25,5,12)$.
  ...
  * Produce a scatterplot of $x$ versus $y$ with the function \texttt{plot()}.


## Factors

A character vector or a factor is define by
...
The data frame \texttt{z} contains two vectors: <tt> sex </tt> and <tt> height </tt>. To print the vector <tt> sex </tt> we use
...
z
```
Use the function <tt> matrix() </tt> to create a $3 \times 3$ matrix from the vector <tt> z </tt>.

```{r}
w <- matrix(z, 3, 3)
```

The matrix <tt>  w </tt> is a $3 \times 3$ matrix
...
In previous sections we created R objects, we used R statements and function in order to define vectors and data frames. In this section we focus on two real datasets: the <tt> airquality </tt> } and the 
law school datasets.\newline
Both datasets are entered to R as data frame. The aim of this section is to illustrate the use of R in working with the data.
...
We can use the function <tt>  par() </tt> to plot the two figures in one page. A general call of the function <tt>  par() </tt> has the form  <tt> par(mfrow=c(number of rows, number of columns)) </tt>.For example <tt>  par(mfrow=c(1,2)) </tt>  will produce a graphical page with two figures in one row as shown in Figure~\@ref(fig:fig2) 
...
## Graphical functions: the law school data

The low school data gives information about LSAT and GPA scores in 82 low schools in USA. It a part of the R package <tt> bootstrap </tt> so you need to install the package. Figure~\@ref(fig:fig3) shows a scatterplot for the scores and a random sample of 15 schools from the population of 82 schools.
...
In this section we focus on R functiona that can be used in order to calculate summary statistics of a sample. The summary statistics that will be discussed 
...
We will use the Chicken Weights (the <tt> chickwts </tt> object ) data frame. The first few lines are shown below
...
A box plot of the  Chicken Weights by diet group is shown in Figure~\@ref(fig:fig5)

```{r fig5,fig.cap="Chicken weihts by diet group."}
boxplot(split(chickwts$weight,chickwts$feed))
```

The mean of the Chicken Weights per group can be calculated using the function <tt> tapply() </tt>. A genela call of the function has the form \newline
<tt> tapply(numerical vector, factor, statistics) </tt>. \newline

For the Chicken Weights dataset we use
...
### Practical session \item Exercise:\newline
The data frame \texttt{mtcars} contains information about fuel consumption of cars.
...
Suppose that we would like to draw a sample of 10 observations from $N(0,1)$ 1000 times. To do this we can use a ``for loop'' in the following way. First, we define a vector that will be used to store the sample means
...
A histogram of the sample means is shown in Figure~\@ref(fig:fig6)
...
The R object <tt>  fun20 </tt> is the function. For the Chicken Weights we have
...
In the same way, the same output (the mean of the sample) can be produced for the wind speed of the aitquality data
